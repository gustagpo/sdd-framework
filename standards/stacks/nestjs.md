...
- **Repositório com `db: any` + dublê de Prisma**: chave inexistente em `select`/`where`/`include` (coluna que a tabela não tem, relação com outro nome) compila, passa em 100 % da unit e **só falha no banco** (`PrismaClientValidationError`, engolida pelo `catch` ⇒ feature inerte com suíte verde). Conferir todo `select`/`where` novo contra o bloco `model` do `schema.prisma`, tipar o cliente (`PrismaClient | Prisma.TransactionClient`) e ter 1 prova contra banco real por repositório novo (N-049).
