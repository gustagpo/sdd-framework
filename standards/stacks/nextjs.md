...
- **Cache de `fetch` do Next mascarando dados stale** → o default de cache de `fetch` GET **mudou entre 14 e 15** (14 cacheia por padrão; 15 não). Não confie no default: seja explícito com `{ cache: 'no-store' }` para dados por-request e `{ next: { revalidate: N } }` para o **Data Cache** por tempo (stale-while-revalidate: o 1º acesso após a janela ainda recebe a versão anterior — a SPEC diz "mínimo N s entre leituras", nunca "até N s"). **O Data Cache só grava respostas 200 e não expira duro**: uma revalidação que volta 404 é descartada e o recurso excluído segue servido até o processo reiniciar — recurso que pode ser excluído exige confirmação `no-store` por idade da entrada (NX-009). ISR de **rota** (`export const revalidate`) grava em `.next/server` — inviável em imagem com sistema de arquivos somente leitura; e `notFound()` sob um `loading.tsx` sai 200 + `noindex` (NX-008). Para queries de banco (fora de `fetch`), use `revalidatePath`/`revalidateTag` após mutações.
